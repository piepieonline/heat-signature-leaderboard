...
export default function ScoreHistogram({ data }: Props) {
  ...
  useEffect(() => {
    ...
    // Buckets: [<=0, 1–99, 100–199, 200–299, 300–399, 400–499, 500–599, 600]
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: (ctx) =>
                ` ${ctx.parsed.y} player${ctx.parsed.y !== 1 ? "s" : ""}`,
            },
          },
        },
        scales: {
  ...
  return (
    <div className="chart-container">
      <h3 className="chart-title">
        Score Distribution{data ? ` — ${data.date}` : ""}
      </h3>
